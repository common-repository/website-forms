    ...
    class WishpondPlugin {
        ...
        if (isset($_POST['submit']) && $_POST['submit']) {
          if (isset($_POST['token'])) {
            ...
            if ($token === get_option('wishpond_auth_token')) {
              $error_message = __('Token value is the same', 'website-forms') . ': ' . $token;
            } else {
              $saved = update_option('wishpond_auth_token', $token, '', 'no');

              if ($saved) {
                $notice_message = __('Successfully updated token', 'website-forms') . ': ' . $token;
              } else {
                $error_message = __('Unable to update token', 'website-forms') . ': ' . $token;
              }
            }
          }
        }
      ...
      public function publish_campaign($data) {
        if (!is_user_logged_in()) {
          wp_die(__('Not logged in', 'website-forms'));
        }

        if (!current_user_can('activate_plugins')) {
          wp_die(__('Not enough permissions', 'website-forms'));
        }

        if (!WishpondUtilities::permalink_structure_valid()) {
          $message = __('Invalid permalink structure. Please go to "Settings"->"Permalinks" and make sure your permalinks use the "postname"', 'website-forms');
          ...
          $action   = __('saved', 'website-forms');

          if ($campaign->post_id !== -1) {
            $saved  = $campaign->update($path);
            $action = __('updated', 'website-forms');
          } else {
            $saved = $campaign->save();
          }
          ...
          if ($saved) {
            ...
            $message = WishpondUtilities::json_message('updated', __('Campaign ', 'website-forms') . $action . __(' successfully. ', 'website-forms') . $campaign_url);
          } else {
            $message = WishpondUtilities::json_message('error', __('An unknown error occurred.', 'website-forms'));
          }
        }
      ...
      public function delete_campaign($data) {
        ...
        if ($campaign) {
          if ($campaign->remove()) {
            $message = WishpondUtilities::json_message('updated', __('Campaign removed successfully.', 'website-forms'));
          } else {
            $message = WishpondUtilities::json_message('error', __('An unknown error occurred.', 'website-forms'));
          }
        } else {
          $message = WishpondUtilities::json_message('error', __('Campaign not found.', 'website-forms'));
        }
      ...
      function wishpond_load_plugin_textdomain() {
        load_plugin_textdomain( 'website-forms', FALSE, basename( dirname( __FILE__ ) ) . '/lang/' );
      }
    }
  }
